...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`
        w-72 bg-primary text-slate-300 h-screen fixed left-0 top-0 flex flex-col border-r border-slate-800 z-40 shadow-xl transition-transform duration-300 ease-in-out
        ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        <div className="p-6 flex flex-col items-center border-b border-slate-800 text-center bg-slate-900/50 relative">
          {/* Mobile Close Button */}
          <button 
            onClick={onClose}
